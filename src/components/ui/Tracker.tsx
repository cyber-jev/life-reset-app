...
import { Save, Download, Image, Plus, Trash2, Edit2 } from 'lucide-react';
...
export default function Tracker({ userId }: TrackerProps) {
  ...
  // Dynamic stats based on current columns
  const getStats = () => {
    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-wrap gap-3 mb-6 justify-between items-center">
        <div className="flex gap-3">
          <button onClick={handleSaveAll} disabled={saveEntriesMutation.isPending} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
            <Save size={18} /> Save All Progress
          </button>
          <button onClick={downloadImage} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
            <Image size={18} /> Download Image
          </button>
        </div>
        <button onClick={() => window.print()} className="flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">
              ...
              <div key={idx} className="border border-gray-200 rounded-xl p-5">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <textarea
                      value={ref.improved}
                      onChange={(e) => { const newRef = [...reflections]; newRef[idx].improved = e.target.value; setReflections(newRef); }}
                      rows={2}
                  ...
                  <div>
                    ...
                    <textarea
                      value={ref.failed}
                      onChange={(e) => { const newRef = [...reflections]; newRef[idx].failed = e.target.value; setReflections(newRef); }}
                      rows={2}
                      className="w-full mt-1 px-3 py-2 border rounded-lg"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => { const newRef = [...reflections]; newRef[idx].change_next_week = e.target.value; setReflections(newRef); }}
                      rows={2}
                      className="w-full mt-1 px-3 py-2 border rounded-lg"
                    />
                  </div>
